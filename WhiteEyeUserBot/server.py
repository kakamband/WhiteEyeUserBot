...
class GoogleImageScraper():
    def __init__(self, webdriver_path, image_path, search_key, number_of_images, headless=False, min_resolution=(0,0), max_resolution=(1920,1080)):
        ...
        if (type(number_of_images)!=int):
            logger.info("GoogleImageScraper Error : Number of images must be integer value.")
            return
        if not os.path.exists(image_path):
            logger.info("GoogleImageScraper Notification : Image path not found. Creating a new folder.")
    ...
    def find_image_urls(self):
        ...
        """
        logger.info("GoogleImageScraper Notification: Scraping for image link... Please wait.")
        image_urls=[]
        count = 0
        options = Options()
        if(self.headless):
        ...
        for indx in range (1,self.number_of_images+1):
            try:
                ...
                logger.info("GoogleImageScraper Skip: Unable to get the link for this photo")
