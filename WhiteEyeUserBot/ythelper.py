#    Copyright (C) DayamZaidi 2020-2021
...
from WhiteEyeUserBot.functions import progress
from WhiteEyeUserBot.functions.FastTelethon import upload_file
from WhiteEyeUserBot.util import WhiteEye_on_cmd, edit_or_reply, sudo_cmd
...
@WhiteEye.on(WhiteEye_on_cmd(pattern="utubevid ?(.*)"))
@WhiteEye.on(sudo_cmd(pattern="utubevid ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    urlissed = event.pattern_match.group(1)
