...
@WhiteEye.on(WhiteEye_on_cmd(pattern="botlist$"))
async def bots(event):
    ...
    for bot in Listed:
        ...
        snt = await WhiteEye.send_message(bot, "/start")
        time.sleep(5)
        msg = await WhiteEye.get_history(bot, 1)
        ...
        await reply.edit(first_msg, parse_mode="html")
        await WhiteEye.read_history(bot)
