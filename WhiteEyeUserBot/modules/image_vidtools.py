...
async def hmm(event):
    ...
    background.save("./whiteeyez/testing.png")

    file_name = "testing.png"
    ok = "./whiteeyez/" + file_name
    await borg.send_file(event.chat_id, ok)
    await hmmu.delete()
    for files in (ok, img):
        if files and os.path.exists(files):
...
async def hmm(event):
    ...
    cv2.imwrite("./whiteeyez/testing.png", gray_img)
    file_name = "testing.png"
    ok = "./whiteeyez/" + file_name
    await borg.send_file(event.chat_id, ok)
    await hmmu.delete()
...
async def img(event):
    ...
    if ";" in text:
        ...
        return
    photo = Image.open("resources/dummy_image/fgs.PNG")
...
async def yufytf(event):
    ...
    await event.edit("`Processing..`")
    text = event.pattern_match.group(2)
    img = Image.open("./resources/zipper.png")
    draw = ImageDraw.Draw(img)
    font = ImageFont.truetype("Fonts/PermanentMarker-Regular.ttf", 220)
    image_widthz, image_heightz = img.size
    w, h = draw.textsize(text, font=font)
    h += int(h * 0.21)
    draw.text(
...
async def warnerstark_s(event):
    ...
    if ws == "flip":
        ...
        warnerstark = "Hehe, Drawn By @WhiteEyedevs"
    await event.delete()
...
async def fasty(event):
    ...
    c_time = time.time()
    cmd = f'ffmpeg -i {hmm} -vf  "setpts=0.25*PTS" FastMotionBy@WhiteEyeDevs.mp4'
    await runcmd(cmd)
    filem = "FastMotionBy@WhiteEyeDevs.mp4"
...
async def fasty(event):
    ...
    hmm = await event.client.download_media(kk.media)
    c_time = time.time()
    cmd = f'ffmpeg -i {hmm} -vf  "setpts=4*PTS" SlowMotionBy@WhiteEyeDevs.mp4'
    await runcmd(cmd)
    filem = "SlowMotionBy@WhiteEyeDevs.mp4"
    if not os.path.exists(filem):
        await event.edit("**Process, Failed !**")
        return
    final_file = await uf(
...
async def flip(event):
    ...
    hmm = await event.client.download_media(kk.media)
    c_time = time.time()
    cmd = f'ffmpeg -i {hmm} -vf "transpose=2,transpose=2" FlipedBy@WhiteEyeDevs.mp4'
    await runcmd(cmd)
    filem = "FlipedBy@WhiteEyeDevs.mp4"
    if not os.path.exists(filem):
        await event.edit("**Process, Failed !**")
        return
    final_file = await uf(
...
async def convert_to_note(event):
    ...
    try:
        ...
        thumb = "./resources/20201201_001148.jpg"
    c_time = time.time()
    filem = "ConvertedBy@WhiteEyeDevs.mp4"
