...
@WhiteEye.on(sudo_cmd(pattern=r"online", allow_sudo=True))
...
@WhiteEye.on(sudo_cmd(pattern="ahelp ?(.*)", allow_sudo=True))
@WhiteEye.on(WhiteEye_on_cmd(pattern="ahelp ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
...
async def cmd_list(event):
    ...
    if not event.text[0].isalpha() and event.text[0] not in ("/", "#", "@", "!"):
        ...
        if tgbotusername is None or input_str == "text":
            ...
            help_string = """WhiteEyeUserBot Modules Are Listed Here !\n
